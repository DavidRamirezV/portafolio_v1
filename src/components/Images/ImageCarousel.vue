<template>
    <div id="image-track" data-mouse-down-at="0" data-prev-percentage="0">
            ...
            <img class="image" v-for="image in itemsProyectos" :src="image.urls.regular" :key="image.id"
                draggable="false" />
        <!--
            <img class="image"
...
export default {
    ...
    ,
    created() {
        axios.get("https://api.unsplash.com/photos/random/", { params: { client_id: 'Gj7t3PNk5Le6lAsN6_WJ_EdSvy7H7YRbiKt0XDLw2DA', query: 'autumn', count: 10, orientation: 'landscape' } }).then(res => {
            this.itemsProyectos = res.data;
        }).catch(error => {
            alert(error)
            console.log(error)
        });
